...
int main()
{
    ...
    // Create the RESPONSE file and KEYS file based on what's in the REQUEST file
    ...
    sprintf(fn_keys, "PQCkemKAT_%d.keys", CRYPTO_SECRETKEYBYTES);
    if ((fp_keys = fopen(fn_keys, "w")) == NULL)
    {
        printf("Couldn't open <%s> for write\n", fn_keys);
        return KAT_FILE_OPEN_ERROR;
    }

    sprintf(fn_ctss, "PQCkemKAT_%d.ctss", CRYPTO_SECRETKEYBYTES);
    if ((fp_ctss = fopen(fn_ctss, "w")) == NULL)
    {
        printf("Couldn't open <%s> for write\n", fn_ctss);
        return KAT_FILE_OPEN_ERROR;
    }
    ...
    do
    {
        ...
        fprintf(fp_keys, "count = %d\n", count);
        fprintBstr(fp_keys, "pk = ", pk, CRYPTO_PUBLICKEYBYTES);
        fprintBstr(fp_keys, "sk = ", sk, CRYPTO_SECRETKEYBYTES);
        fprintf(fp_keys, "\n");

        fclose(fp_keys); // Close the KEYS file after writing
